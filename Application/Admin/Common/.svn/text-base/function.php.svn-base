 ...
 *
 */
function int_to_string(&$data,$map=array('status'=>array(1=>'正常',-1=>'删除',0=>'禁用',2=>'未审核',3=>'草稿'),'type'=>array(1=>'后台充值',2=>'在线充值'),'pay_type'=>array(1=>'余额支付',2=>'在线支付'))) {
...
/**
* 获取关键词查询结果条数
*   key:SHENSOU_RESULT_KEYWORDS_LIST
*
*/
function get_keywords_result_count($keyword){
	$keyword = str_replace("新词:","",$keyword);
	$keyword = str_replace("旧词:","",$keyword);
	$redis = new \Redis();
	$redis->pconnect(C('REDIS_HOST'), 6379);
	$count = $redis->zcard(C('KEY_RESULT_KEYWORDS_PRE_FIX').md5($keyword));
	return $count ? $count : 0;
}

function loadVM($tpl,$config){
	while (strpos($tpl, "{\$") > 0) {  
		$counts = strpos($tpl, "{\$");  
		$counte = strpos($tpl, "}");  
		$substr = substr($tpl, $counts +2, $counte - $counts -2);  
		$tpl = str_replace("{\$" . $substr . "}", $config[$substr], $tpl);  
	} 
	return $tpl;
}
...
/**
* 按时间统计查询 
* $day: today\week\month\quarter\year\ 默认等于所有
*/
function find_create_time($day='',$data){
	//查询当天数据
	if($day=='today'){
		...
		$arr=getdate();
		$num=$arr['wday'];
		if(date("w")==0){//周日算上一周，-7
			$lastweek = -7;
		}else{
			$lastweek = 0;
		}
        $start = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1+$lastweek,date("Y"));
        $end = mktime(23,59,59,date("m"),date("d")-date("w")+7+$lastweek,date("Y"));
		$data['cid']=$cid;
 ...
 * 邮件发送函数
 */
function Send2Mail($to, $subject = '', $body = '', $attachment = null){
	...
	$mail->AddAddress($to, $config['FROM_NAME']);
