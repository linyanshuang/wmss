...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|外贸神搜管理平台</title>
...
<body>
	...
	<div class="cf">
		<div class="fl">
            ...
            <button class="btn ajax-post confirm" url="<?php echo U('User/nologinNotify');?>" target-form="ids">邮件通知</button>
			
			<button class="btn"  id="aweeknologin" >最近<?php echo C('N_DAY_NOLOGIN');?>天未登录用户</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="hidden" name="last_login_time" id="last_login_time"  value="<?php echo I('last_login_time');?>" >
				<input type="text" name="nickname" class="search-input" value="<?php echo I('nickname');?>" placeholder="请输入用户昵称或者ID">
				<input type="hidden" name="_order" id="_order"  value="<?php echo I('_order');?>" >
				<input type="hidden" name="_field" id="_field"  value="<?php echo I('_field');?>" >
				<input type="hidden" name="_ordertext" id="_ordertext"  value="<?php echo I('searchordertext');?>" >
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span><?php echo ($vo['expiryend']); ?></span></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
			    <a href="<?php echo U('Deposit/doDeposit?uid='.$vo['uid']);?>" class="">充值</a>
				<a href="<?php echo U('Payment/renew?uid='.$vo['uid']);?>" class="">续费</a>
				<a href="<?php echo U('User/resetPassword?id='.$vo['uid']);?>" class="confirm ajax-get">重置密码</a>
				...
				<a href="<?php echo U('User/updatenickname?uid='.$vo['uid']);?>" class="">修改邮箱</a>
	...
	$("#aweeknologin").click(function(){
		$("#last_login_time").val("<?php echo C('N_DAY_NOLOGIN');?>");
		gosearch();
	});
	if("<?php echo I('_field');?>"!='.'){
		$("[_field=<?php echo I('_field');?>]").append("<?php echo I('_ordertext');?>");
	}
