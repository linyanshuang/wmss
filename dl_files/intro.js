(function(window,document){
    ...
    try {
    	...
    	//欢迎页更新时间，定值获取方法 new Date().getTime();
		var updateTime = 1481779400090;
		
        if (localStorage) {
        	if(localStorage.firstIntoTime){
        		if(localStorage.firstIntoTime>=updateTime){
        			localStorage.setItem(x5Version + 'introPage',false);
        		} else{
        			localStorage.firstIntoTime = new Date().getTime();
        			localStorage.setItem(x5Version + 'introPage',true);
        		}
        	} else{
        		localStorage.firstIntoTime = updateTime;
        		localStorage.setItem(x5Version + 'introPage',true);
        	}
        	
        	if(localStorage.getItem(x5Version + 'introPage') === true || 
        			localStorage.getItem(x5Version + 'introPage') === "true"){
        		createIntroElement();
